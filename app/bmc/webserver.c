/**
 * @file webserver.c
 * @brief BMC Web Service
 *
 * This service uses GoAhead Webs server and the BMC common interface
 *
 * @author wenyi (wenyi0421@163.com)
 * @author Michael J Feher (PhearZero)
 * @bug No Known Bugs
 *
 ...
 */
static int error_response(Webs *wp, int code, char *errstr) {
    ...
    cJSON_AddStringToObject(pItem, "message", errstr);
...
/**
 * @brief Check sdcard mount status
 *
 * Look for a mounted card with a valid partition at /dev/mmcblk0p1
 *
 * @return True/False based on sdcard state
 */
...
static int set_usbmode(Webs *wp) {
    ...
    if (NULL == node || NULL == mode) {
        error_response(wp, 400, "mode or node is null");
        return -1;
    }
    env_usb_t *usb = &(env_get_ctx()->usb);
    usb->mode = atoi(mode);
...
static void bmcdemo(Webs *wp) {
    printf("Running V1 BMC Action: [%s] - %s \n", wp->method, wp->path);
    char *pOpt = NULL;
    char *pType = NULL;
    ...
    if (NULL == pOpt || NULL == pType) {
        error_response(wp, 400, "opt type error");
        return;
    }
    ...
    if (strcasecmp(pOpt, "get") == 0) {
        ...
        error_response(wp, 401, "opt error");
    }
}
 ...
 *
 * Handle requests to /api/nodes and route them to the appropriate request handler
 *
 * @path /api/node
...
int webstart() {
    ...
    start_goahead(
            ...
            "/mnt/auth.txt",
            g_func_reg,
            sizeof(g_func_reg) / sizeof(go_asp_func_t)
    );
